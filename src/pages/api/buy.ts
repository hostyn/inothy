...
async function buy(
  ...
  const createPayInResponse = await mangopay.PayIns.create({
    AuthorId: user.data.mangopayClientId as string,
    CreditedWalletId: user.data.mangopayWalletId as string,
    PaymentType: 'CARD',
  ...
  if (createPayInResponse.Status === 'CREATED') {
    ...
    const transaction: FirestoreTransaction = {
      ...
      authorWalletId: user.data.mangopayWalletId as string,
    ...
    const recipts = await Promise.all(
      documents.map(async document => {
        ...
        const recipt = await mangopay.Transfers.create({
          AuthorId: user.data.mangopayClientId as string,
          DebitedWalletId: user.data.mangopayWalletId as string,
          CreditedWalletId: ownerData.mangopayWalletId as string,
          DebitedFunds: {
            Currency: 'EUR',
            Amount:
              (isAmbassador
