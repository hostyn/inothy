...
export async function isUsernameAvailable(username: string): Promise<boolean> {
  if (username.length === 0) throw new Error('username-required')
...
export async function getSubject(
  ...
  if (subjectId.length === 0) throw new Error('subject-id-required')
...
export async function getDocument(
  ...
  if (subjectId.length === 0) throw new Error('subject-id-required')
  if (docId.length === 0) throw new Error('document-id-required')
...
export async function getUser(userId: string): Promise<GetUserResponse> {
  if (userId.length === 0) throw new Error('user-id-required')
...
export async function updateBankAccount(
  ...
  if (iban.length === 0) throw new Error('iban-required')
...
export async function addReferral(ref: string): Promise<void> {
  if (ref.length === 0) throw new Error('ref-required')
